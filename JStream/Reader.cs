...
public class Reader
{
    ...
    {
        if (IsNull) return Variant.Null;
        if (IsFalse) return new Variant(false);
        if (IsTrue) return new Variant(true);
        if (IsNumber) return new Variant(Number);
        if (IsString) return new Variant(StringValue);
        return Variant.Null;
    }
}